...
install_jdk() {
  ...
  echo "BEGINNING TO SOURCE jvm-common"

  echo "sourcing /tmp/jvm-common/bin/util"
  source /tmp/jvm-common/bin/util
  echo "sourcing /tmp/jvm-common/bin/java"
  source /tmp/jvm-common/bin/java
  echo "sourcing /tmp/jvm-common/opt/jdbc.sh"
  source /tmp/jvm-common/opt/jdbc.sh
  echo "jvm-common.install.time ${start}"
  mtime "jvm-common.install.time" "${start}"

  let start=$(nowms)
  echo "INVOKING install_java_with_overlay"
